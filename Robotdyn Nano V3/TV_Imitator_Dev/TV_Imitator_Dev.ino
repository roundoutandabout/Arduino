...
unsigned long previousMillis_scn = 0; // Counter for checking interval to switch to new scene
unsigned long previousMillis[] = {previousMillis_red, previousMillis_blu, previousMillis_grn, previousMillis_wht, previousMillis_alm, previousMillis_scn};
...
short interval_scene = 0; // Interval of switching to another scene (abrupt change of all colors)

short intervalfreeze_red = 10; // Interval of freezing on particular PWM level
short intervalfreeze_grn = 10;
short intervalfreeze_blu = 10;
short intervalfreeze_wht = 10;
...
void loop()
{
	...
	switch (mode) {
		...
		break;
		case 4: // Beautiful smooth changing of colours. Not suitable for TV imitation. To make change of colors look like TV light, take code from function tvImitatorTask()

			ledSmooth(red, 2, 5, 0, 255, 1000, 3001, 25, 230);
			ledSmooth(grn, 2, 5, 0, 255, 1000, 3001, 25, 230);
			ledSmooth(blu, 2, 5, 0, 255, 1000, 3001, 25, 230);
			ledSmooth(wht, 2, 5, 0, 255, 1000, 3001, 25, 230);
...
void tvImitatorTask() {
	if (trigger) {
		if(currentMillis - previousMillis[scn] < interval_scene) { // Check if time interval is less then interval of scene change
			ledSmooth(red, 1, 4, 0, 100, 1000, 1501, 25, 75);
			ledSmooth(grn, 1, 4, 0, 100, 1000, 1501, 25, 75);
			ledSmooth(blu, 1, 4, 0, 255, 1000, 1501, 25, 230);
			ledSmooth(wht, 1, 4, 0, 255, 1000, 1501, 25, 230);
		} else { // If time interval is bigger, change scene
			interval_scene = random(3000, 6000); // Set new interval of switching to new scene
			switchScene();
			previousMillis[scn] = currentMillis;
		}
	}
}
...
void ledSmooth(byte color, byte minInterval, byte maxInterval, byte minPWM, byte maxPWM, short minFreezeInterval, short maxFreezeInterval, byte minFreezePWM, byte maxFreezePWM) // This function gradually changes brightness level of color. minInterval and maxInterval define min and max possible time intervals of changing of the brightness level (bigger maxInterval means that brightness level will be able ti change slower). minPWM and maxPWM are minimum and maximum brightness levels which colors will be able to reach. minFreezeInterval and maxFreezeInterval are minimum and maximum time intervals for which colors will be able to freeze (stay at fixed brightness level). minFreezePWM and maxFreezePWM are minimum and maximum brightness levels at which colours will be able to freeze.

//To avoid situation when colors can not freeze, minPWM should be less than minFreezePWM. maxPWM should be bigger than maxFreezePWM

{

	if(currentMillis - previousMillis[color] > intervals[color]) { // If time interval of changing brightness is reached
		...
		if (pwmfreezes[color] == prevpwms[color]) { // if current pwm "prevpwm" is equal to pwm that must be freezed "prevpwm"
			if (currentMillis-previousMillis[color] > intervalfreezes[color]) { // if freezed pwm level stayed for specified time interval "intervalfreeze", we generate new interval of freezing and new pwm level of freezing.
				...
				pwmfreezes[color] = random(minFreezePWM, maxFreezePWM); // Set new level of freezing the brightness
				newpwms[color] = random(minPWM, maxPWM); // Set new level of brightness level to gradually move to
		...
		} else { // if current pwm is not equal to pwm that must be freezed, we continue to change pwm level, until it will be equal to pwmfreezes[color] again

			// Brightness level will fix to pwmfreezes[color] only if prevpwms[color] will reach pwmfreezes[color] during gradually moving to newpwms[color]
...
void switchScene() {
		...
		prevpwms[red] = random(0, 100);

		prevpwms[grn] = random(0, 100);

		prevpwms[blu] = random(0, 250);

		prevpwms[wht] = random(0, 250);

		analogWrite(ledpins[red], prevpwms[red]);
		analogWrite(ledpins[grn], prevpwms[grn]);
		analogWrite(ledpins[blu], prevpwms[blu]);
		analogWrite(ledpins[wht], prevpwms[wht]);
}
